...
- **No browser running** — the harness does not auto-launch on Windows (qualified, browser-use 0.13.10): with the remote-debugging prerequisite satisfied it reports `chrome-not-running` (start the browser, then retry); without it, the daemon log carries the `enable chrome://inspect/#remote-debugging` diagnostic. Start the browser, then retry the call.
